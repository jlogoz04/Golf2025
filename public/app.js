  ...
  if(!currentUser){
    ...
    $('#login-btn').onclick = async ()=>{
      ...
      try{
        ...
        start(); // rebuild nav and pages with auth
      }catch(e){
    ...
    const tabs = $$('#nav .tab');
    tabs.forEach((t)=>{
      if(t.textContent==='Admin') t.style.display = isAdmin? 'inline-flex' : 'none';
    });
...
async function renderPersonStats(playerId){
  ...
  const netData = curve.map(d=> d.net);
  ...
  rounds.forEach(r=>{
    ...
    const table = `<table><thead><tr><th>Hole</th><th>Par</th><th>Score</th></tr></thead><tbody>${rows}</tbody><tfoot><tr><th>Total</th><th>${parTotal}</th><th>${r.total}</th></tr></tfoot></table>`;
    ...
    if(canDelete){
      ...
      inner.appendChild(spacer); inner.appendChild(del);
    }
...
async function renderAdminProfiles(){
  ...
  target.innerHTML = `
    ...
    <div class="card"><div class="hd"><h2>Log a score (any player)</h2></div>
      <div class="bd">
        ...
        <span id="adm-log-msg" class="hint" style="margin-left:12px"></span>
      </div>
    </div>
  `;
  ...
  async function buildAdmLogTable(){
    ...
    wrap.innerHTML = `<table><tbody>${rows.join('')}</tbody></table>`;

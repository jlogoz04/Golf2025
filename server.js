...
app.get('/api/admin/users', authRequired, adminOnly, async (req,res)=>{
  const { rows } = await q(`
    SELECT 
      u.id, u.username, u.is_admin, u.is_disabled, u.player_id,
      p.name,
      COALESCE((SELECT COUNT(*) FROM rounds r WHERE r.player_id = u.player_id), 0) AS rounds
    FROM users u
    LEFT JOIN players p ON p.id = u.player_id
    ORDER BY u.is_admin DESC, p.name NULLS LAST, u.username ASC
  `);
  res.json(rows);
});
...
app.patch('/api/admin/users/:id', authRequired, adminOnly, async (req,res)=>{
  const { id } = req.params;
  const { is_disabled, password, name } = req.body || {};

  const { rows: urows } = await q('SELECT id, username, player_id, is_admin FROM users WHERE id=$1', [id]);
  if(urows.length === 0) return res.status(404).json({ error: 'user not found' });
  const u = urows[0];
  if(u.username === 'jlogozzo') return res.status(400).json({ error: 'cannot modify the core admin user' });

  // Update users table fields
  const parts = []; const vals = []; let idx = 1;
  if(typeof is_disabled === 'boolean'){ parts.push(`is_disabled = $${idx++}`); vals.push(is_disabled); }
  if(typeof password === 'string' && password.length){
    const hash = await bcrypt.hash(password, 12);
    parts.push(`password_hash = $${idx++}`); vals.push(hash);
  }
  if(parts.length){
    vals.push(id);
    await q(`UPDATE users SET ${parts.join(', ')} WHERE id = $${idx}`, vals);
  }

  // Optional: update player display name
  if(typeof name === 'string' && name.trim().length && u.player_id){
    await q('UPDATE players SET name=$1 WHERE id=$2', [name.trim(), u.player_id]);
  }
...
app.delete('/api/admin/users/:id', authRequired, adminOnly, async (req,res)=>{
  const { id } = req.params;
  const { rows: urows } = await q('SELECT id, username, player_id FROM users WHERE id=$1', [id]);
  if(urows.length === 0) return res.status(404).json({ error: 'user not found' });
  const u = urows[0];
  if(u.username === 'jlogozzo') return res.status(400).json({ error: 'cannot delete the core admin user' });

  // Delete user first, then delete their player (cascades rounds)
  await q('DELETE FROM users WHERE id=$1', [id]);
  if(u.player_id){
    await q('DELETE FROM players WHERE id=$1', [u.player_id]);
  }
  res.json({ ok: true });
});

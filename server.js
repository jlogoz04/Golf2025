...
// Admin creates a new user profile (player + user)
...
app.get('/api/admin/users', authRequired, adminOnly, async (req,res)=>{
  const { rows } = await q(`
    SELECT u.id, u.username, u.is_admin, u.is_disabled, u.player_id, p.name AS name,
           COALESCE((SELECT COUNT(*) FROM rounds r WHERE r.player_id = u.player_id), 0) AS rounds
      FROM users u
      LEFT JOIN players p ON p.id = u.player_id
     ORDER BY u.is_admin DESC, p.name NULLS LAST, u.username
  `);
  res.json(rows);
});
...
app.patch('/api/admin/users/:id', authRequired, adminOnly, async (req,res)=>{
  const { id } = req.params;
  const { is_disabled, password } = req.body || {};
  const { rows: urows } = await q('SELECT username FROM users WHERE id=$1', [id]);
  if(urows.length===0) return res.status(404).json({ error: 'not found' });
  if(urows[0].username === 'jlogozzo') return res.status(400).json({ error: 'cannot modify admin core account' });

  if(typeof is_disabled === 'boolean'){
    await q('UPDATE users SET is_disabled=$1 WHERE id=$2', [is_disabled, id]);
  }
  if(password){
    const hash = await bcrypt.hash(password, 12);
  ...
  const { rows } = await q(`
    SELECT u.id, u.username, u.is_admin, u.is_disabled, u.player_id, p.name AS name,
           COALESCE((SELECT COUNT(*) FROM rounds r WHERE r.player_id = u.player_id), 0) AS rounds
      FROM users u LEFT JOIN players p ON p.id=u.player_id WHERE u.id=$1`, [id]);
  res.json(rows[0]);
});

app.delete('/api/admin/users/:id', authRequired, adminOnly, async (req,res)=>{
  const { id } = req.params;
  const { rows: urows } = await q('SELECT username, player_id FROM users WHERE id=$1', [id]);
  if(urows.length===0) return res.status(404).json({ error: 'not found' });
  if(urows[0].username === 'jlogozzo') return res.status(400).json({ error: 'cannot delete admin core account' });
  const playerId = urows[0].player_id;
  await q('DELETE FROM users WHERE id=$1', [id]);
  if(playerId){
    await q('DELETE FROM players WHERE id=$1', [playerId]);
  }
  res.json({ ok: true });
});
